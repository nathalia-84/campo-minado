...
Bool area_valida(tabuleiro ** mat, int l, int c, int l_entrada, int c_entrada){
  ...
  if (mat[l][c].bomba) return false;
  if (l==l_entrada && c==c_entrada) return false;
  if (l==l_entrada+1 && c==c_entrada) return false;
  if (l==l_entrada-1 && c==c_entrada) return false;
  if (l==l_entrada && c==c_entrada+1) return false;
  if (l==l_entrada && c==c_entrada-1) return false;
  if (l==l_entrada+1 && c==c_entrada+1) return false;
  if (l==l_entrada-1 && c==c_entrada-1) return false;
  if (l==l_entrada+1 && c==c_entrada-1) return false;
  if (l==l_entrada-1 && c==c_entrada+1) return false;
...
void salvar_bombas(tabuleiro **  mat, int l_entrada, int c_entrada) {
  ...
  while(i<qtd) {
    l = rand() % N;
    c = rand() % M;
    while (l==l_entrada && c==c_entrada || mat[l][c].bomba) { //while??
      l = rand() % N;
      c = rand() % M;
    }
    mat[l][c].bomba = true;
...
void processar_primeira_rodada(tabuleiro ** mat, time_t p_tempo){
  ...
  int M = COLUNAS;
  int l, c;
  int decisao = menu_primeira_rodada();
