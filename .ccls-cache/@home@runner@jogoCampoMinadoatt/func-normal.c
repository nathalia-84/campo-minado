...
/*
tabuleiro ** matriz_copia(tabuleiro ** mat){
  int N = LINHAS;
  int M = COLUNAS;
  tabuleiro ** matriz_copia = (tabuleiro**) malloc((N)*sizeof(tabuleiro*)); // n linhas
  for(int i=0;i<N;i++){
    matriz_copia[i] = (tabuleiro*) malloc((M)*sizeof(tabuleiro)); // n colunas
    for(int j=0; j<M, j++) {
      if (mat[i][j].clicado || mat[i][j].marcado) {
        matriz_copia[i][j] = mat[i][j];
      } 
    }
  }
  return matriz_copia;
