...
void processar_jogo (tabuleiro ** mat, Bool jogo_existente){
  ...
  */
  ...
  // se o jogador chegou ao final de uma partida, qualquer jogo salvo existente será excluído
...
Processamento processar_rodada(tabuleiro ** mat, time_t p_tempo){
  /*
  Esta função processa a decisão do jogador em cada rodada (clicar, marcar, parar), e retorna se a decisão levou o jogador a perder, parar, ou ontinuar no jogo.
  */
  int N = LINHAS;
  int M = COLUNAS;
  int l, c;
...
void iniciar_jogo() {
  ...
  if( arquivo == NULL ){
    ...
    char resposta[3];
    printf("Existe um jogo já iniciado, deseja continuá-lo?\n");
    printf("Digite Sim ou Não.\n");
    ler_linha_com_terminador(resposta, 4, stdin);
    char sim[4] = "sim";
    if (comparar(resposta, sim)) {
      ler_matriz(mat);
      percorrer_matriz(mat);
      imprimir_matriz(mat);
      processar_jogo(mat, jogo_existente);
