    ...
    for(int j=-1 ; j<=1 ; j++){
      if(celula_valida(l+i, c+j, N, M)){
        if(!mat[l+i][c+j].clicado){
          ...
          printf("têm baixas chances de ser uma bomba\n");
          r[0] = l+i;
          r[1] = c+j;
          return r;
        }
...
int *pedir_ajuda(tabuleiro ** mat){
  ...
  */
  ...
  /* quando não há nenhuma célula marcada como limpa, e já existem células marcadas como bombas, para escolher uma célula, a função irá percorrer a matriz buscando alguns critérios. Escolherá a célula com a menor quantidade de bombas ao redor (ctrl_qtd) e a maior quantidade de células não clicadas e não marcadas (ctrl_dif). Ao selecionar a célula, retornará a primeira posição adjascente válida */
